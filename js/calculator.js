function renderCalculatorContent() {
    ...
    if (!form) return; // Do nothing if we are not on the calculator page
    ...
    fetch('calculator-services.json')
        ...
        .then(data => {
            for (const categoryKey in data) {
                ...
                categoryHeader.innerHTML = `
                    <span data-i18n="calculator_services.${categoryKey}.category_title">${i18n.t('calculator_services.' + categoryKey + '.category_title')}</span>
                ...
                data[categoryKey].forEach(service => {
                    ...
                    label.innerHTML = `
                        <span>
                            <span class="font-bold" data-i18n="calculator_services.${categoryKey}.${service.key}">${i18n.t('calculator_services.' + categoryKey + '.' + service.key)}</span>
                            <span class="text-sm text-gray-400 block" data-i18n="calculator_services.${categoryKey}.${service.key}_desc">${i18n.t('calculator_services.' + categoryKey + '.' + service.key + '_desc')}</span>
                        </span>
        ...
        .catch(error => {
            ...
            servicesList.innerHTML = `<p class="text-red-500">${i18n.t('calculator_page.load_error')}</p>`;
        });
